...
class GridTest extends TestCase {
  "Grid.set" should "disallow setting same digit in same row, column or box" in {
    forAll { (value: Digit, row: Digit, col: Digit, x: Digit) =>
      ...
      assert(col == x || grid.flatMap(_.set(GridKey.rowCol(row, x), value)).isEmpty)
      assert(row == x || grid.flatMap(_.set(GridKey.rowCol(x, col), value)).isEmpty)
    }
  }
  ...
  it should "disallow setting same digit in same box" in {
    forAll { (value: Digit, box: Digit, num: Digit, x: Digit) =>
      ...
      assert(num == x || grid.flatMap(_.set(GridKey.boxNum(box, x), value)).isEmpty)
    }
  }

  it should "allow setting another digit in same row or column" in {
    forAll { (value: Digit, newValue: Digit, row: Digit, col: Digit, x: Digit) =>
      val grid = Grid.empty().set(GridKey.rowCol(row, col), value)
      assert(col == x || newValue == value || !grid.flatMap(_.set(GridKey.rowCol(row, x), newValue)).isEmpty)
      assert(row == x || newValue == value || !grid.flatMap(_.set(GridKey.rowCol(x, col), newValue)).isEmpty)
    }
  }

  it should "allow setting another digit in same box" in {
    forAll { (value: Digit, newValue: Digit, box: Digit, num: Digit, x: Digit) =>
      val grid = Grid.empty().set(GridKey.boxNum(box, num), value)
      assert(num == x || newValue == value || !grid.flatMap(_.set(GridKey.boxNum(box, x), newValue)).isEmpty)
    }
  }

  "Grid.fillBox" should "work" in {
    val grid = Grid.empty().set(
    ...
    assert(grid.map(Grid.fillBox(1)) == grid.flatMap(_.set(GridKey.rowCol(3, 3), 1)))
  }

  "Grid.fillBoxes" should "work" in {
    val grid = Grid.empty().set(
      GridKey.rowCol(1, 1) -> 2,
      GridKey.rowCol(1, 4) -> 1,
      GridKey.rowCol(2, 7) -> 1,
    ...
    assert(grid.map(Grid.fillBoxes) == grid.flatMap(_.set(GridKey.rowCol(3, 3), 1)))
  }
  ...
  "Grid.fillCols" should "work" in {
    ...
    assert(Grid.fillCols(grid) == grid.set(GridKey.rowCol(3, 1), 7).get)
  }
  ...
  "Grid.fillSingleAlloweds" should "work" in {
    ...
    assert(Grid.fillSingleAlloweds(grid) == grid.set(GridKey.rowCol(1, 1), 5).get)
  }

  "Grid.fromString" should "work" in {
    val expected = Grid.empty().set(
      GridKey.rowCol(1, 1) -> 2,
      GridKey.rowCol(1, 4) -> 1,
